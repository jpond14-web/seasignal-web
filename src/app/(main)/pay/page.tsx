    ...
    <div className="max-w-2xl mx-auto">
      ...
      {tab === "explore" ? (
        <div className="space-y-4">
          <div>
            <label className="block text-sm text-slate-300 mb-1.5">Rank *</label>
            <input
              type="text"
              value={rank}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm text-slate-300 mb-1.5">Vessel Type</label>
              <select
                value={vesselType}
            ...
            <div>
              <label className="block text-sm text-slate-300 mb-1.5">Flag State</label>
              <input
                type="text"
                value={flagState}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm text-slate-300 mb-1.5">Rank *</label>
              <input type="text" required value={form.rank} onChange={(e) => setForm({ ...form, rank: e.target.value })}
                className="w-full px-3 py-2.5 bg-navy-800 border border-navy-600 rounded text-slate-100 text-sm focus:border-teal-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-sm text-slate-300 mb-1.5">Vessel Type *</label>
              <select required value={form.vessel_type} onChange={(e) => setForm({ ...form, vessel_type: e.target.value as Enums<"vessel_type"> })}
                className="w-full px-3 py-2.5 bg-navy-800 border border-navy-600 rounded text-slate-100 text-sm focus:border-teal-500 focus:outline-none">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm text-slate-300 mb-1.5">Flag State</label>
              <input type="text" value={form.flag_state} onChange={(e) => setForm({ ...form, flag_state: e.target.value })} placeholder="e.g. Panama"
                className="w-full px-3 py-2.5 bg-navy-800 border border-navy-600 rounded text-slate-100 placeholder:text-slate-500 text-sm focus:border-teal-500 focus:outline-none" />
            </div>
            <div>
          ...
          <div>
            <label className="block text-sm text-slate-300 mb-1.5">Monthly Base (USD) *</label>
            <input type="number" required min="0" value={form.monthly_base_usd} onChange={(e) => setForm({ ...form, monthly_base_usd: e.target.value })}
              placeholder="e.g. 4500"
              className="w-full px-3 py-2.5 bg-navy-800 border border-navy-600 rounded text-slate-100 placeholder:text-slate-500 text-sm focus:border-teal-500 focus:outline-none" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm text-slate-300 mb-1.5">Overtime Structure</label>
              <input type="text" value={form.overtime_structure} onChange={(e) => setForm({ ...form, overtime_structure: e.target.value })} placeholder="e.g. Fixed OT included"
                className="w-full px-3 py-2.5 bg-navy-800 border border-navy-600 rounded text-slate-100 placeholder:text-slate-500 text-sm focus:border-teal-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-sm text-slate-300 mb-1.5">Leave Pay</label>
              <input type="text" value={form.leave_pay} onChange={(e) => setForm({ ...form, leave_pay: e.target.value })} placeholder="e.g. 8 days/month"
                className="w-full px-3 py-2.5 bg-navy-800 border border-navy-600 rounded text-slate-100 placeholder:text-slate-500 text-sm focus:border-teal-500 focus:outline-none" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm text-slate-300 mb-1.5">Contract (months)</label>
              <input type="number" min="1" max="24" value={form.contract_duration_months} onChange={(e) => setForm({ ...form, contract_duration_months: e.target.value })}
                className="w-full px-3 py-2.5 bg-navy-800 border border-navy-600 rounded text-slate-100 text-sm focus:border-teal-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-sm text-slate-300 mb-1.5">Year *</label>
              <input type="number" required min="2020" max="2030" value={form.year} onChange={(e) => setForm({ ...form, year: e.target.value })}
                className="w-full px-3 py-2.5 bg-navy-800 border border-navy-600 rounded text-slate-100 text-sm focus:border-teal-500 focus:outline-none" />
            </div>
          </div>
